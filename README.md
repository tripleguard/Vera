...
| Что | Где | Формат |
|------|------|--------|
| Профиль, факты, диалог, сессия | `data/memory.json` | JSON |
| Legacy (миграция при первом запуске) | `data/MEMORY.md` | Markdown |
...
Вера, запомни мой email name@domain.com
...
Агент может выполнять сложные и алгоритмические запросы, используя систему инструментов. В случае комплексных запросов, задача декомпозируется и выполняется по шагам с передачей контекста:
...
постепенно использовать преимущества `niquests` без резкого переписывания
поиска.

1. Запрос отправляется через Brave Search.
2. При возникновении сетевых сбоев, ошибок HTTP или возвращении 0 результатов от Brave, система автоматически переключается на резервный поиск через DuckDuckGo Lite (для обеспечения непрерывности работы без API-ключей).
3. Получение списка релевантных ссылок.
4. Параллельная загрузка страниц через общий HTTP-клиент `niquests`/`requests`.
5. Очистка текста от мусора (скриптов, рекламы, меню).
6. Суммаризация собранного контекста с помощью LLM для формирования точного ответа.

## Конфигурация
Файл: `data/config.json`
Ключевые параметры: настройки слова активации, таймаут тишины, голос и параметры синтеза речи (`tts.voice_name`, `tts.total_steps`, `tts.volume`), внешние серверы LLM и алиасы сайтов.
...
|------|--------|---------------|
| `tests/test_bm25.py` | 14 | BM25-индекс: токенизация, лёгкий стемминг (RU/EN), ранжирование, lifecycle (add/remove/replace/clear/auto-rebuild) |
| `tests/test_memory.py` | 38 | MemoryManager: API contract, миграция из MEMORY.md, инференс категорий, поиск (BM25/recency), pin, лимиты, context cap, dedup, eviction |
| `tests/test_e2e_memory.py` | 14 | End-to-end: путь `extract_from_remember_command` → `MemoryManager.add_fact` → `get_context_for_prompt` → JSON round-trip, полностью симулирующий `main/agent.py:807-940` |

### Запуск отдельного модуля
```bash
python tests/test_bm25.py
python tests/test_memory.py
python tests/test_e2e_memory.py
```
...
- **Нет звука:** проверьте правильность имени голоса в `tts.voice_name` и доступные аудиоустройства воспроизведения.
...
- **Память «забывает» факты:** проверьте, что `data/memory.json` не повреждён; при первом запуске после обновления старая `MEMORY.md` мигрирует автоматически. Лимит — 20 фактов; закреплённые (pinned) факты сохраняются последними при эвикции.
