...
| Категория | Функции |
|----------|---------|
| Интерфейс | Плавающий виджет, текстовый чат, сессии, проекты, рабочая панель, логи в реальном времени и шесть тем оформления |
...
| LLM-управление | Переключение мышления, токенный бюджет, подключение внешних LLM-серверов |
...
Вера, запомни мой город Нальчик
Вера, погода
Вера, курс доллара
...
Вера, открой ютуб
```
Погода определяется по указанному городу. Если город в запросе не назван, используется поле профиля `город` из памяти пользователя. Профиль можно заполнить обычной командой памяти, например: «Вера, запомни мой город Нальчик». В ответе показываются текущая температура, погодные условия и короткая рекомендация по одежде или осадкам.
