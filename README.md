...
| Категория | Функции |
|----------|---------|
| Интерфейс | Плавающий виджет, текстовый чат, сессии, проекты, рабочая панель, логи в реальном времени и несколько тем оформления |
...
| Надежность | Автоперезапуск backend, экран готовности агента, уведомления о сбоях и защищенный IPC-мост Electron |
...
- **Панель сессий:** Каждая сессия хранится отдельно, поддерживает переименование, закрепление, архивирование и удаление. При переключении агент подгружает только контекст выбранной сессии.
- **Проекты:** Созданные презентации и документы доступны в отдельном разделе и открываются напрямую из приложения.
...
- **Нижняя панель:** Показывает активную модель и версию приложения, открывает периодические задачи и поток системных логов.
- **Темы оформления:** Набор светлых и темных тем выбирается в настройках интерфейса.
...
|---------|---------------|
| Аудио и мультимодальность | Нормализация громкости TTS, отсутствие клиппинга, поиск основной GGUF-модели и передача изображений |
| Память и сессии | Дедупликация, лимиты, закрепленные факты, изоляция сессий, архивирование и перенос старой истории |
...
| Веб-экстракция | Извлечение основного контента через `trafilatura` и fallback на `BeautifulSoup` |
